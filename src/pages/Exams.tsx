    ...
    <div className="space-y-6">
            ...
            <CardContent className="space-y-3">
              {topPerformers.length > 0 ? topPerformers.map((winner, idx) => (
                <div key={idx} className="flex items-center gap-2 p-2 bg-white/60 rounded-lg border border-amber-100">
                  ...
                  <div className="flex-1"><p className="font-black text-slate-800 text-[11px] leading-none">{winner.name}</p><p className="text-[9px] font-bold text-amber-600 uppercase tracking-tighter">مقفل: {winner.subject}</p></div>
                </div>
      ...
      <Dialog open={openDialog} onOpenChange={(val) => { setOpenDialog(val); if(!val) setEditId(null); }}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="space-y-1"><Label className="font-black text-sm">المجموعة</Label><Select value={form.group} onValueChange={(val) => setForm({ ...form, group: val })}><SelectTrigger className="font-bold"><SelectValue placeholder="اختر المجموعة" /></SelectTrigger><SelectContent>{groups.map(g => <SelectItem key={g.id} value={g.name} className="font-bold">{g.name}</SelectItem>)}</SelectContent></Select></div>
      ...
      <Dialog open={openGrading} onOpenChange={setOpenGrading}>
        <DialogContent className="max-w-2xl p-0 overflow-hidden rounded-2xl">
          <div className="bg-slate-900 p-6 text-white flex justify-between items-center"><div><h2 className="text-xl font-black">{selectedExam?.subject}</h2><p className="text-slate-400 font-bold text-sm">{selectedExam?.group} | {selectedExam?.totalMarks} درجة</p></div><BookOpen className="w-8 h-8 opacity-20" /></div>
          <div className="p-6 max-h-[50vh] overflow-y-auto divide-y">
            {students.filter(s => s.group === selectedExam?.group).map(student => (
              <div key={student.id} className="py-4 flex items-center justify-between hover:bg-slate-50 px-2 transition-colors rounded-lg">
                ...
                <div className="flex items-center gap-2">
                  <Input type="number" className="w-20 h-10 text-center font-black border-2 text-lg" value={selectedExam?.grades?.[student.id] ?? ""} onChange={(e) => updateStudentGrade(student.id, +e.target.value)} />
                  <span className="font-bold text-muted-foreground text-xs">من {selectedExam?.totalMarks}</span>
                </div>
              </div>
            ))}
          </div>
          <div className="p-4 bg-slate-50 flex justify-end"><Button className="font-black px-10 h-11" onClick={() => setOpenGrading(false)}>حفظ وإغلاق</Button></div>
        </DialogContent>
      </Dialog>
    </div>
  );
